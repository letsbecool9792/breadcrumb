...
Everything above the V1 boundary below is in scope. Everything under "Post-V1" is explicitly out.

## V1 must ship

**Capture**
- Android share sheet target (`ACTION_SEND` / `ACTION_SEND_MULTIPLE`) — text, links, images, PDFs
- Quick Settings tile → transparent activity → save clipboard
- Source-app provenance captured on every save
- Save writes to Room and returns instantly; WorkManager handles the rest

**Processing**
- On-device ML Kit OCR at save time for immediate local searchability
- Single Gemini Flash multimodal call producing structured JSON (text, description, entities, type, dates)
- Embedding generation, stored in Atlas at 768 or 1536 dims

**Retrieval**
- Search-bar-first UI. No dashboard, no feed, no folder tree.
- Flash-Lite query parsing into semantic query + filters
- Hybrid search via `$rankFusion` with Atlas pre-filtering
- Ranked result list; tapping a result opens the original artifact

**Backend**
- Express: one ingest endpoint, one search endpoint. It stays thin.
...
**Seeding — treat as a week-one feature, not a nice-to-have**
- A bulk importer: screenshots folder, Chrome bookmarks export, saved media, downloaded PDFs.
- **Why this is not optional:** a personal memory search engine is worthless until it holds a few hundred items. The likely failure mode for this project is building it, using it a week, having 14 items, finding search unimpressive, and losing motivation. This kills most apps in the category. Seed 500+ real items on day one so retrieval is tuned against a corpus that actually exercises it.

## V1 explicitly excludes

Voice notes. They are in the product vision and the pipeline already handles audio in principle (rule #3), but recording UI, playback and audio storage are their own slice of work. Add them once text and images are solid.
